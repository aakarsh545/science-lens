...
interface DiagramProps {
  ...
  // Expose step controls UI for interactive diagrams
  const StepControls = () => (
    <div className="flex items-center justify-center space-x-2 mt-2">
      <Button size="sm" variant="outline" onClick={stepBack}>Previous</Button>
      <Button size="sm" variant="outline" onClick={stepForward}>Next</Button>
      ...
      <Card className="p-6">
        ...
        for (const line of lines) {
          if (emojiRegex.test(line)) {
            ...
            // Remove known emoji markers using regex for performance
            currentTitle = line.replace(emojiRegex, '').trim() || 'Key Concept';
      {/* Section Navigation */}
      {sections.length > 1 && (
        <div className="flex space-x-2 overflow-x-auto pb-2">
